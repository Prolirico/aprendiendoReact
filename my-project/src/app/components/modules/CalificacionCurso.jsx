...
const CalificacionModal = ({
  ...
  const toggleActividadExpansion = (index) => {
  ...
  const handleActividadChange = (index, field, value) => {
    ...
    if (field === "porcentaje") {
      nuevasActividades[index][field] = Number.parseInt(value, 10) || 0;
    } else {
  ...
  const handleRemoveActividad = (index) => {
    ...
    const reajustadas = {};
    Object.keys(nuevasExpandidas).forEach((key) => {
      const numKey = Number.parseInt(key);
      if (numKey > index) {
        reajustadas[numKey - 1] = nuevasExpandidas[key];
      } else if (numKey < index) {
        reajustadas[numKey] = nuevasExpandidas[key];
      }
    });
    setActividadesExpandidas(reajustadas);
  };
  ...
  const materialesPorActividad = useMemo(() => {
    ...
    if (materialExistente.actividad) {
      materialExistente.actividad.forEach((material) => {
        // Agrupamos por el ID de la actividad, que ahora viene del backend
        if (material.id_actividad) {
          if (!agrupados[material.id_actividad])
            agrupados[material.id_actividad] = [];
          agrupados[material.id_actividad].push(material);
        }
      });
    }
    return agrupados;
  ...
  const handleSave = () => {
    ...
    actividades.forEach((_, index) => {
      colapsadas[index] = false;
    });
    setActividadesExpandidas(colapsadas);

    // Llamamos a onSave y le pasamos una función para actualizar el estado
    // desde el padre, una vez que la operación asíncrona termine.
    onSave({
      ...curso,
      ...
      planeacion,
      materiales,
    }).then(({ actividadesActualizadas, conteoSubidos }) => {
      // Hidratar las actividades con los campos que faltan
      const actividadesConCamposFrontend = actividadesActualizadas.map(
        (act) => ({
          ...act,
          ...
          enlace_recursos: [],
          tipos_permitidos:
            act.tipos_permitidos ||
            JSON.parse(act.tipos_archivo_permitidos || '["pdf", "link"]'),
        }),
      );
      setActividades(actividadesConCamposFrontend);

      // Colapsar todas las actividades después de actualizar
      const colapsadas = {};
      actividadesConCamposFrontend.forEach((_, index) => {
        colapsadas[index] = false;
      });
      setActividadesExpandidas(colapsadas);

      // Limpiar formularios de subida si se subió algo
      if (conteoSubidos.materiales > 0) {
        setMateriales([]);
      }
      if (conteoSubidos.planeacion > 0) {
        setPlaneacion((prev) => ({ ...prev, archivo_temario: null }));
      }
      // Forzar la recarga del material existente para ver los cambios inmediatamente
  ...
  useEffect(() => {
    if (curso.id_curso) {
      // Resetear estado inicial al abrir modal
      setActividades([]);
      setActividadesExpandidas({});
      setUmbral(60); // valor por defecto
    }
  ...
  // Función para cargar configuración existente del curso
  const loadConfiguracionExistente = async () => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        `${API_BASE_URL}/api/calificaciones/${curso.id_curso}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      if (response.ok) {
        ...
        if (data.actividades && data.actividades.length > 0) {
          const actividadesConCamposFrontend = data.actividades.map((act) => ({
            ...act,
            fecha_limite: act.fecha_limite || "",
            instrucciones: act.instrucciones || "",
            archivo_instrucciones: "",
          ...
          // Colapsar todas las actividades después de cargarlas
          const colapsadas = {};
          actividadesConCamposFrontend.forEach((_, index) => {
            colapsadas[index] = false;
          });
          ...
          // Si no hay actividades existentes, crear una por defecto
          const actividadPorDefecto = {
            nombre: "Tarea 1",
            porcentaje: 100,
            fecha_limite: "",
            instrucciones: "",
          ...
          setActividades([actividadPorDefecto]);
          setActividadesExpandidas({ 0: false });
        }
        if (data.umbral_aprobatorio) {
          setUmbral(data.umbral_aprobatorio);
        }
      }
    } catch (error) {
  ...
  // Cargar material existente y configuración al abrir el modal
  useEffect(() => {
    if (curso.id_curso) {
      loadMaterialExistente();
  ...
  const handleRemoveArchivoActividad = (actividadIndex, archivoIndex) => {
    const nuevasActividades = [...actividades];
    nuevasActividades[actividadIndex].archivos_instrucciones.splice(
      archivoIndex,
      1,
    );
    setActividades(nuevasActividades);
  ...
  const handleFileChange = (e, type, index = null) => {
    ...
    if (type === "planeacion") {
      ...
      const nuevasActividades = [...actividades];
      const archivoIndex = nuevasActividades[
        index
      ].archivos_instrucciones.findIndex((f) => f === null);
      if (archivoIndex !== -1) {
            ...
            <div className={styles.actividadesTab}>
              ...
              {actividades.map((act, index) => (
                <div key={index} className={styles.actividadItem}>
                  <div
                    className={styles.actividadHeader}
                    onClick={() => toggleActividadExpansion(index)}
                  >
                    <div className={styles.actividadTitleRow}>
                      <span className={styles.actividadNombre}>
                        {act.nombre || `Actividad ${index + 1}`}
                      </span>
                      <div className={styles.actividadControls}>
                        <span className={styles.actividadPorcentaje}>
                          {act.porcentaje}%
                        </span>
                        <FontAwesomeIcon
                          icon={
                  ...
                  {actividadesExpandidas[index] && (
                    <div className={styles.actividadContent}>
                      ...
                      <div className={styles.actividadRecursos}>
                        <h4>
                          Recursos adicionales para esta actividad (Opcional)
                        </h4>

                        {/* Material existente para esta actividad */}
                        {act.id_actividad &&
                          materialesPorActividad[act.id_actividad] &&
                          materialesPorActividad[act.id_actividad].length >
                            0 && (
                            <div className={styles.existingMaterial}>
                              <h5>Recursos ya subidos:</h5>
                              {materialesPorActividad[act.id_actividad].map(
                                (item) => (
                                  <div
                                    key={item.id_material}
                                    className={styles.materialCard}
                                  >
                                    <div className={styles.materialInfo}>
                                      <span className={styles.materialName}>
                                        <FontAwesomeIcon
                                          icon={
                                            item.es_enlace ? faLink : faFilePdf
                                          }
                                          className={
                                            item.es_enlace
                                              ? styles.linkIcon
                        ...
                        <div className={styles.recursoOption}>
                          ...
                          {act.archivos_instrucciones.map(
                                ...
                                <div className={styles.fileUploadArea}>
                                  <input
                                    ...
                                    onChange={(e) => {
                                      ...
                                      nuevasActividades[
                                        index
                                      ].archivos_instrucciones[archivoIndex] =
                                        e.target.files[0];
                                      setActividades(nuevasActividades);
                                    }}
                                    className={styles.fileInput}
...
const CalificacionCurso = ({ rol, entidadId }) => {
  ...
  const handleSaveCurso = async (cursoActualizado) => {
    ...
    try {
      ...
      // 4. Subir recursos de las actividades (archivos y enlaces)
      // Usamos las actividades del frontend que tienen los archivos y enlaces
      // pero mapeamos con los IDs del backend
      for (const actividadOriginal of cursoActualizado.actividades || []) {
        // Encontrar la actividad correspondiente en la respuesta del backend para obtener el ID
        const actividadBackend = (dataCalif.actividades || []).find(
          (act) =>
            act.id_actividad === actividadOriginal.id_actividad ||
            act.nombre === actividadOriginal.nombre,
        );

        if (!actividadBackend) {
          console.warn(
            `⚠️ No se encontró actividad en backend para: ${actividadOriginal.nombre}`,
          );
          continue;
        }
        ...
        console.log(
          `🔍 Debug archivos para ${actividadBackend.nombre}:`,
          actividadOriginal.archivos_instrucciones,
        );
        for (const archivo of actividadOriginal.archivos_instrucciones || []) {
          if (!archivo || archivo === null) {
            console.log(
              `⏭️ Saltando archivo null/undefined para: ${actividadBackend.nombre}`,
            );
            continue;
          }
          console.log(
            `📎 Subiendo archivo de apoyo para: ${actividadBackend.nombre}`,
          );
          ...
          formDataRecurso.append("id_actividad", actividadBackend.id_actividad); // <-- VINCULACIÓN CLAVE
          formDataRecurso.append("nombre_archivo", archivo.name);
          formDataRecurso.append(
            "descripcion",
            `Archivo de apoyo para la actividad: ${actividadBackend.nombre}`,
          );
          formDataRecurso.append("es_enlace", "false");

          fetch(`${API_BASE_URL}/api/material`, {
            method: "POST",
            headers: {
              Authorization: `Bearer ${token}`,
            },
            ...
            .then((res) => {
              if (!res.ok)
                console.warn(
                  `⚠️ Error al subir recurso para: ${actividadBackend.nombre}`,
                );
              else
                console.log(
                  `✅ Recurso de apoyo subido para: ${actividadBackend.nombre}`,
                );
            })
            .catch((err) =>
              console.error(`💥 Error de red subiendo recurso: ${err}`),
            );
        }

        // Guardar cada enlace de recurso
        console.log(
          `🔍 Debug enlaces para ${actividadBackend.nombre}:`,
          actividadOriginal.enlace_recursos,
        );
        for (const enlace of actividadOriginal.enlace_recursos || []) {
          if (!enlace || !enlace.trim()) {
            console.log(
              `⏭️ Saltando enlace vacío para: ${actividadBackend.nombre}`,
            );
            continue;
          }
          console.log(
            `🔗 Guardando enlace de recurso para: ${actividadBackend.nombre}`,
          );
          ...
          formDataEnlace.append("id_actividad", actividadBackend.id_actividad); // <-- VINCULACIÓN CLAVE
          formDataEnlace.append(
            "nombre_archivo",
            `${actividadBackend.nombre} - Enlace de apoyo`, // Nombre genérico para enlaces
          );
          formDataEnlace.append(
            "descripcion",
            `Enlace de apoyo para la actividad: ${actividadBackend.nombre}`,
          );
          ...
          fetch(`${API_BASE_URL}/api/material`, {
            ...
            .then((res) => {
              if (!res.ok)
                console.warn(
                  `⚠️ Error al guardar enlace para: ${actividadBackend.nombre}`,
                );
              else
                console.log(
                  `✅ Enlace de apoyo guardado para: ${actividadBackend.nombre}`,
                );
            })
            .catch((err) =>
              console.error(`💥 Error de red guardando enlace: ${err}`),
            );
        }
      }
      ...
      conteoSubidos.planeacion = cursoActualizado.planeacion?.archivo_temario
        ? 1
        : 0;
      conteoSubidos.materiales = (cursoActualizado.materiales || []).filter(
        (m) => (m.tipo === "pdf" && m.archivo) || (m.tipo === "link" && m.link),
      ).length;
      conteoSubidos.actividades = (cursoActualizado.actividades || []).filter(
        (a) =>
          (a.archivos_instrucciones &&
            a.archivos_instrucciones.some(
              (archivo) => archivo !== null && archivo !== undefined,
            )) ||
          (a.enlace_recursos &&
            a.enlace_recursos.some((enlace) => enlace && enlace.trim())),
      ).length;

      console.log("🎉 Proceso completado exitosamente");
      showToast("¡Configuración guardada exitosamente!", "success");

      // Devolvemos las actividades actualizadas, pero "hidratadas" con los campos del frontend
      const actividadesHidratadas = (dataCalif.actividades || []).map(
        (actividadServidor) => {
          // Encontrar la actividad original en el estado del frontend para no perder la referencia
          const actividadOriginal = cursoActualizado.actividades.find(
            (a) =>
              a.id_actividad === actividadServidor.id_actividad ||
              a.nombre === actividadServidor.nombre,
          );

          return {
            ...actividadServidor, // Datos del servidor (con el id_actividad correcto)
            // Asegurar que todos los campos necesarios están presentes y no son null
            fecha_limite: actividadServidor.fecha_limite || "",
            instrucciones: actividadServidor.instrucciones || "",
            archivo_instrucciones: "",
            // Campos que solo existen en el frontend, los restauramos como arrays vacíos
            archivos_instrucciones: [],
            enlace_recursos: [],
            tipos_permitidos:
              actividadServidor.tipos_permitidos ||
              JSON.parse(
                actividadServidor.tipos_archivo_permitidos || '["pdf", "link"]',
              ),
          };
        },
      );

      // Actualizar el curso seleccionado para forzar recarga de datos
      if (cursoSeleccionado) {
        setCursoSeleccionado({
          ...cursoSeleccionado,
          _reloadTrigger: Date.now(),
          // Forzar que el modal recargue los datos al reabrirse
          actividades: actividadesHidratadas,
          umbral_aprobatorio: cursoActualizado.umbral_aprobatorio,
        });
      }

      return { actividadesActualizadas: actividadesHidratadas, conteoSubidos };
    } catch (err) {
      console.error("💥 Error durante el guardado:", err);
      showToast(`Error al guardar: ${err.message}`, "error");
      // En caso de error, devolvemos el estado original para no romper la UI
      return {
        actividadesActualizadas: cursoActualizado.actividades,
        conteoSubidos: { planeacion: 0, materiales: 0, actividades: 0 },
      };
    }
  };
        ...
        {cursoSeleccionado && (
          <CalificacionModal
            ...
            onClose={() => {
              ...
              // Forzar recarga de datos del curso al cerrar
