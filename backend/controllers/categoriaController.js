  ...
  try {
    const [categorias] = await pool.query(
      `SELECT 
       ...
       LEFT JOIN areas_conocimiento ac ON cc.id_area = ac.id_area
       ORDER BY ac.nombre ASC, cc.orden_prioridad ASC, cc.nombre_categoria ASC`,
    );
    res.json(categorias);
  } catch (error) {
...
// @desc    Obtener todas las categorías de un área específica
// @route   GET /api/categorias/area/:id
// @access  Private (para admins)
const getCategoriasByArea = async (req, res) => {
  const { id } = req.params;
  try {
    const [categorias] = await pool.query(
      `SELECT * FROM categoria_curso 
       WHERE id_area = ? 
       ORDER BY orden_prioridad ASC, nombre_categoria ASC`,
      [id],
    );
    res.json(categorias);
  } catch (error) {
    console.error(`Error al obtener las categorías del área ${id}:`, error);
    res.status(500).json({ error: "Error interno del servidor." });
  }
};
...
const createCategoria = async (req, res) => {
  const { nombre_categoria, descripcion, id_area } = req.body;
  ...
  try {
    ...
    const [result] = await connection.query(
      "INSERT INTO categoria_curso (nombre_categoria, descripcion, orden_prioridad, id_area) VALUES (?, ?, ?, ?)",
      [nombre_categoria, descripcion, newOrder, id_area],
    );
...
const updateCategoria = async (req, res) => {
  const { id } = req.params;
  const { nombre_categoria, descripcion, estatus, orden_prioridad, id_area } =
    req.body;
  const newOrder = orden_prioridad ? parseInt(orden_prioridad, 10) : undefined;

  if (!nombre_categoria || !estatus || !id_area) {
    return res
      .status(400)
  ...
  try {
    ...
    await connection.query(
      "UPDATE categoria_curso SET nombre_categoria = ?, descripcion = ?, estatus = ?, orden_prioridad = ?, id_area = ? WHERE id_categoria = ?",
      [
        nombre_categoria,
        descripcion,
        estatus,
